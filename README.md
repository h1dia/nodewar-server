...
「黒い砂漠」の拠点戦用カウントダウンタイマーアプリケーションです。
ギルドメンバーと共有して、敵の砦が破壊されるまでの時間をリアルタイムで管理・共有できます。

## 🚀 機能

- **リアルタイムタイマー**: 設定された時刻までのカウントダウンを表示します。
- **時刻同期**: timeapi.ioから取得した時刻を基準に、全員が同じ残り時間を見られるようにしています。
- **音声通知**: 設定時間が来ると音が鳴ります。
- **管理画面**: タイマーの追加・削除、遅延秒数の調整が可能です（要Basic認証）。
